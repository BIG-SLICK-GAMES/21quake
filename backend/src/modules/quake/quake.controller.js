...
function handleQUAKEError(error, res, next) {
  if (error?.statusCode) {
...
async function listLeaderboard(req, res, next) {
  try {
    ...
    return handleQUAKEError(error, res, next);
  }
}
...
async function startQUAKEGame(req, res, next) {
  try {
...
async function completeQUAKEGame(req, res, next) {
  try {
...
async function QUAKEProfile(req, res, next) {
  try {
    const data = await getQUAKEProfile(req.user);
    return res.json({
      data,
      message: 'success',
    });
...
async function saveQUAKERun(req, res, next) {
  try {
