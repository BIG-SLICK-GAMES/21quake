...
export interface HubQUAKEEntry {
  _id?: string;
...
export interface HubQUAKEWeeklyEntry {
  _id?: string;
  ...
  totalQUAKEChipsWon: number;
  weekEndDate: string;
  weekStartDate: string;
}

export interface HubQUAKEWeeklyLeaderboard {
  entries: HubQUAKEWeeklyEntry[];
  prizePreview: {
    first: number;
    second: number;
    third: number;
...
export interface HubQUAKEDailyStatus {
  chipBalance?: number;
...
export interface HubQUAKEStartResponse {
...
export interface HubQUAKECompleteResponse {
  cardsUsed: number;
  currentChipBalance?: number;
  difficulty: "easy" | "medium" | "hard";
  made21: boolean;
...
export interface HubQUAKERunPayload {
  adjustedScore: number;
  bonusMultiplier: number;
  buyIn: number;
  difficulty: "easy" | "medium" | "hard";
...
export interface HubQUAKEProfile {
  recentRuns: HubQUAKEEntry[];
  stats: {
