...
import { getQUAKEAppearance, QUAKEAppearance } from "./appearance";
import { useDeviceProfile } from "../hooks/useDeviceProfile";
import { QUAKERoutes } from "../navigation/routes";
import { hubQUAKEApi } from "../platform/api/quake";
...
const QUAKEAppearanceContext = createContext<QUAKEAppearance | null>(null);
...
function useQUAKEAppearance() {
  const context = useContext(QUAKEAppearanceContext);

  if (!context) {
    throw new Error("useQUAKEAppearance must be used inside GameExperience.");
  }
...
export function GameExperience() {
  ...
  const appearance = getQUAKEAppearance(settings);
  ...
  const [dailyFreeGames, setDailyFreeGames] = useState<Record<SetupDifficulty, number>>({
    easy: QUAKE_FREE_GAMES_PER_DIFFICULTY_DAILY,
    hard: QUAKE_FREE_GAMES_PER_DIFFICULTY_DAILY,
    medium: QUAKE_FREE_GAMES_PER_DIFFICULTY_DAILY
  });
  ...
  const canAffordDifficulty = (difficulty: SetupDifficulty) =>
    ...
    (Number(profile?.nChips) || 0) >= QUAKE_EXTRA_GAME_COST;
  ...
  useEffect(() => {
    ...
    const redirect = setTimeout(() => {
      router.replace(QUAKERoutes.leaderboard);
    }, 1400);
  ...
  useEffect(() => {
    async function syncKeepAwake() {
      try {
        if (settings.keepAwake) {
          await activateKeepAwakeAsync("QUAKE-session");
        } else {
          await deactivateKeepAwake("QUAKE-session");
        }
      } catch {}
    }
    ...
    void syncKeepAwake();
    return () => {
      void deactivateKeepAwake("QUAKE-session").catch(() => {});
    };
  }, [settings.keepAwake]);
  ...
  useEffect(() => {
    ...
    async function hydrateDailyStatus() {
      ...
      try {
        const response = await hubQUAKEApi.getDailyStatus(token);
        if (cancelled) {
          return;
        }
        ...
        if (!cancelled) {
          setEconomyMessage("Daily Quake status is unavailable.");
        }
      }
    }
  ...
  useEffect(() => {
    ...
    async function completeSession() {
      try {
        const response = await hubQUAKEApi.completeGame(authToken, {
          cardsUsed,
          finalTotal,
  ...
  async function startRun(
    ...
    if (status === "authenticated" && token) {
      ...
      try {
        const response = await hubQUAKEApi.startGame(token, difficultyOverride);
        ...
        const message =
          error instanceof Error
            ? error.message
            : "You need 50 chips to play another Quake game";
      ...
      <View style={[styles.bannerMenuPanel, { width: Math.min(frameWidth - outerPad * 2, 360) }]}>
        ...
        <View style={styles.bannerMenuGrid}>
          <Pressable
            onPress={() => {
              setMenuOpen(false);
              router.push(QUAKERoutes.leaderboard);
            }}
            style={({ pressed }) => [
          ...
          <Pressable
            onPress={() => {
              setMenuOpen(false);
              router.push(QUAKERoutes.profile);
            }}
            style={({ pressed }) => [
              styles.bannerMenuMiniButton,
              pressed && styles.bannerMenuButtonPressed
          ...
          <Pressable
            onPress={() => {
              setMenuOpen(false);
              router.push(QUAKERoutes.settings);
            }}
            style={({ pressed }) => [
              styles.bannerMenuMiniButton,
              pressed && styles.bannerMenuButtonPressed
          ...
          <Pressable
            onPress={() => {
              setMenuOpen(false);
              router.replace(QUAKERoutes.lobby);
            }}
            style={({ pressed }) => [
              styles.bannerMenuMiniButton,
              pressed && styles.bannerMenuButtonPressed
      ...
      >
        <View style={styles.bannerHeader}>
          ...
          <View style={styles.bannerHeaderActions}>
            ...
            <Pressable
              onPress={() => router.replace(QUAKERoutes.lobby)}
              style={({ pressed }) => [
                styles.bannerExitButton,
                pressed && styles.bannerExitButtonPressed
              ]}
  ...
  if (isMobile) {
    return (
      <QUAKEAppearanceContext.Provider value={appearance}>
      ...
      </QUAKEAppearanceContext.Provider>
    );
  }
