...
import type { HubQUAKEProfile } from "../platform/types";
...
const STORAGE_KEY = "21-quake/leaderboard";
...
export async function loadLeaderboard(options?: LeaderboardOptions) {
  if (options?.useRemote) {
    try {
      const response = await hubQUAKEApi.getLegacyLeaderboard(MAX_ENTRIES);
...
export async function saveLeaderboardEntry(
  ...
  if (options?.useRemote && options.token) {
    try {
      await hubQUAKEApi.saveRun(options.token, {
...
export async function loadLeaderboardProfile(token?: string | null) {
  ...
  try {
    const response = await hubQUAKEApi.getProfile(token);
    const data = response.body.data as HubQUAKEProfile;
