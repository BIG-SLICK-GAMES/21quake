...
import type { HubQUAKEWeeklyEntry } from "../platform/types";
...
export function LeaderboardScreen() {
  ...
  const [weeklyEntries, setWeeklyEntries] = useState<HubQUAKEWeeklyEntry[]>([]);
  ...
  useFocusEffect(
    useCallback(() => {
      ...
      async function hydrate() {
        ...
        let nextWeeklyEntries: HubQUAKEWeeklyEntry[] = [];

        try {
          const weeklyResponse = await hubQUAKEApi.getLeaderboard(24);
...
function LeaderboardTable({
  ...
  emptyBody: string;
  entries: Array<LeaderboardEntry | HubQUAKEWeeklyEntry>;
...
function getEntryId(entry: LeaderboardEntry | HubQUAKEWeeklyEntry, index: number) {
...
function formatMetric(entry: LeaderboardEntry | HubQUAKEWeeklyEntry, kind: "lines" | "time" | "weekly" | "winners") {
  if (kind === "weekly" && "totalQUAKEChipsWon" in entry) {
    return formatChipCount(entry.totalQUAKEChipsWon);
  }
