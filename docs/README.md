...
- `backend/`: QUAKE-owned API files for game-specific services
...
- QUAKE-specific backend ownership starts in `backend/`, so your backend developer can extend game APIs there without editing the 21 Holdem codebase.
...
- QUAKE API routes
- QUAKE models and services
- shared-database connection code
- future QUAKE auth/profile adapters if you decide to stop using the common backend
...
2. QUAKE backend changes go in `backend/`
3. Database and infra changes go in `database/`

Do not add new QUAKE server logic back into the old 21 Holdem backend repo unless it is intentionally shared across all games.
...
Optional QUAKE-local DB bootstrap:
...
Only use the QUAKE-local DB bootstrap if the shared Holdem docker stack is not already running. It uses the same `bigslick-mongodb`/`bigslick-redis` service names and the same local ports so QUAKE does not drift onto a separate database.
